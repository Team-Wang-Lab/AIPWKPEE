...
delta <- f.theta(data$z)+2*data$x
pis = f.pis(data$u)

type ="aipw"; x=data$x;y=data$y;z=data$z;zgrid = seq(min(z),max(z),length.out = 20);
tol=0.1;maxit=3;family=gaussian(link = "identity")

a<-ebbs(fun=type, x=z,y=y,xpar = x,betapar = 2,h1a=h1a, h1b=h1b, xgrid = zgrid, r=r,pis=pis,delta=delta,gridallx=TRUE)

mean(abs(a$f-f.theta.true(a$xgrid))/f.theta.true(a$xgrid))
...
# 
 # sim.aipw <- ebbs(fun="aipw", x=data$z,y=data$y,xpar=data$x,betapar = 2,h1a=0.07, h1b=0.15, xgrid = seq(min(z),max(z),length.out = 30),
 #                  r=data$r,pis=pis,delta=delta)
 # sim.aipw1 <- ebbs(fun="aipw", x=data$z,y=data$y,xpar=data$x,betapar = 2,h1a=0.07, h1b=0.15, xgrid = seq(min(z),max(z),length.out = 30),
 #                  r=data$r,pis=pis,delta=delta,gridallx = FALSE)
 # sim.ipw <- ebbs(fun="ipw", x=data$z,y=data$y,xpar=data$x,betapar = 2,h1a=0.07, h1b=0.15, xgrid = seq(min(z),max(z),length.out = 30),
 #                   r=data$r,pis=pis,delta=delta,gridallx = FALSE)
#  sim.naive <- ebbs(fun="naive", x=data$z,y=data$y,xpar=data$x,betapar = 2,h1a=0.07, h1b=0.15, xgrid = seq(min(z),max(z),length.out = 20),
#                   r=data$r,pis=pis,delta=delta)
# # 
# # # #grid analysis
# # zgrid = seq(0.05,0.95,length.out = 30)
# # #
#  ipw <- lpoly(z,y,xpar=data$x,betapar = 2,mean(band.z(sim.aipw,z=seq(0.05,0.95,length.out = 30))),seq(min(z),max(z),length.out = 30),p,type="ipw",r=r,pis=pis,delta = delta)
...
#  naive <- lpoly(z,y,xpar=data$x,betapar = 2,mean(band.z(sim.naive,z=seq(0.05,0.95,length.out = 30))),seq(min(z),max(z),length.out = 30),p,type="naive",r=r,pis=pis,delta = delta)
# mean(ipw$var)
# mean(aipw$var)
...
# mean(abs(naive$f-f.theta.true(zgrid))/f.theta.true(zgrid))
